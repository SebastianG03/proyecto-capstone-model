...
class PlayerNumberDetector:
    ...
    def __init__(self, model_dir: str):
        base = VisionEncoderDecoderModel.from_pretrained("microsoft/trocr-base-printed")
    ...
    def _crop_dorsal_region(self, frame: np.ndarray, bbox: Sequence[int]) -> np.ndarray:
        ...
        y_top, y_bottom = int(y1 + box_h * self.VERT_FRAC_TOP), int(y1 + box_h * self.VERT_FRAC_BOT)
        x_left, x_right = int(x1 + box_w * self.HORZ_MARGIN), int(x2 - box_w * self.HORZ_MARGIN)
    ...
    def predict_batch(self, crops: List[np.ndarray]) -> List[Tuple[Optional[int], float]]:
        if not crops:
            return []

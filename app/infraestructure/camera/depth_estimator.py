...
from app.entities.interfaces.depth_model_types import DepthModelsTypes
from app.utils.routes import DEPTH_MODEL_PATH
...
class DepthEstimator:
    ...
    def __init__(
        ...
        self.logger.info(f"[DepthCalculator] Cargando modelo MiDaS: {depth_model_type}")
        ...
        self.logger.info("[DepthCalculator] Inicializando Pose Landmarker (API Tasks)")
        self._init_pose_landmarker()
            
        self.logger.info("[DepthCalculator] Modelos cargados correctamente")
    ...
    def _get_waist_center(self, roi_frame: np.ndarray) -> Optional[tuple[float, float]]:
        ...
        try:
            ...
            self.logger.warning("[DepthCalculator] No se encontraron landmarks de cadera")
            return None
    ...
    def process_player_depth(
        ...
        self.logger.info(f"[DepthCalculator] Dimensiones del frame: {w_frame}x{h_frame}")
    ...
    def _calculate_depth_at_point(self, frame: np.ndarray, x: int, y: int) -> float:
        ...
        self.logger.debug(f"[DepthCalculator] Profundidad en ({x}, {y}): {depth_value}, tipo de valor {type(depth_value)}")
        self.logger.debug(f"[DepthCalculator] Escala de profundidad: {self.depth_scale}, tipo de valor {type(self.depth_scale)}")

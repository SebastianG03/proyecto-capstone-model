...
class BallTracker(Tracker):
    ...
    def _extract_ball(self, detections: sv.Detections, cls_names_inv: dict):
        try:
            ...
            info_logger.info(f"[BallTracker] Clases disponibles: {cls_names_inv}")
            info_logger.info(f"[BallTracker] Detecciones - class_ids: {detections.class_id}")
            info_logger.info(f"[BallTracker] Detecciones - confianza: {detections.confidence}")
            xyxy = np.asarray(getattr(detections, "xyxy", None))
            info_logger.info(f"[BallTracker] xyxy: {xyxy}")
            class_ids = np.asarray(getattr(detections, "class_id", None))
            info_logger.info(f"[BallTracker] class_ids: {class_ids}")
            if xyxy is None or class_ids is None:
                info_logger.info("[BallTracker] No xyxy o class_ids")
                return None
            info_logger.info("[BallTracker] Buscando clase 'ball'")
            ball_idx = cls_names_inv.get("ball")
            if ball_idx is None:
                return None
            mask = class_ids == ball_idx
            info_logger.info(
                f"[BallTracker] mask: {mask} resultante de class ids "
                f"{class_ids} y ball idx{ball_idx}"
            )
            if not mask.any():
                info_logger.info(
                    "[BallTracker] Ningún balón detectado, intentando recuperar posición"
                )
    ...
    def _recover_ball_position(
        self, detections: sv.Detections, cls_names_inv: dict
        ...
        if self.age < self.max_age and not np.allclose(self.kf.x[:2], 0):
            ...
            print(
                f"[BallTracker] Usando predicción de Kalman: ({predicted_x}, {predicted_y})"
            )
        ...
        if player_idx is not None:
            ...
            if player_mask.any():
                ...
                if player_centers:
                    ...
                    info_logger.info(
                        "[BallTracker] Balón estimado por posición de jugadores:"
                        f"({ball_x}, {ball_y})"
                    )

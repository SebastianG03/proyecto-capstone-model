...
class ProcessTimeReporter:
    def __init__(self, match_id: int, logger: Logger | None = None):
    ...
    def _create_report_file(self, match_id: int):
        try:
            self.report_file = Path(OUTPUT_REPORTS_DIR, "time_report", f"time_report_{match_id}.json")
            ...
            self.logger.info(f"[Time Reporter] Archivo de reporte de tiempo creado: {self.report_file}")
        except:
            self.logger.error(f"[Time Reporter] Error al crear el archivo de reporte de tiempo: {traceback.format_exc()}")
            raise
    ...
    def publish(self):
        ...
        self.logger.info(f"[Time Reporter] Reporte de tiempo publicado en {self.report_file}")

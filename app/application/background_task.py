...
async def process_run(db: Session, video_name: str, match_id: int, color: str):
    try:
        ...
        _ = await export_data(db, match_id, start_time=start, heatmaps=heatmaps or {}, color=color)
    except Exception as e:
...
async def export_data(
    ...
    try:
        ...
        # player_output_file.write_text(json.dumps(player_stats, indent=2, ensure_ascii=False), encoding="utf-8")
        ball_output_file.write_text(json.dumps(ball_export_data, indent=2, ensure_ascii=False), encoding="utf-8")
        
        debug_logger.debug(f"Notificando emision de estadisticas para match_id {match_id}...")
        resp = httpx.post(
        ...
        debug_logger.debug(f"Notificación enviada, respuesta: {resp.status_code} - {resp.text}")
